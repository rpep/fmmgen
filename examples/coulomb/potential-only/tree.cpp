...
*/
Tree build_tree(double *pos, double *q, size_t nparticles, size_t ncrit, size_t order, double theta) {
  ...
  tree.M.resize(tree.cells.size() * Nterms(order), 0.0);
  tree.L.resize(tree.cells.size() * Nterms(order), 0.0);
  for(size_t i = 0; i < tree.cells.size(); i++) {
    tree.cells[i].M = &tree.M[i*Nterms(order)];
    tree.cells[i].L = &tree.L[i*Nterms(order)];
  }
  return tree;
}
...
void Tree::compute_field_fmm(double *F) {
  for(size_t i = 0; i < FMMGEN_OUTPUT_SIZE*particles.size(); i++) {
    F[i] = 0.0;
  }
  clear_M();
  clear_L();

...
def test_L_shift_0_order_monopole_source():
    ...
    L_dict, _ = gen.generate_mappings(
        order - source, symbols, key="grevlex", source_order=source
    )
...
def test_L_shift_1_order_monopole_source():
    ...
    assert (
        exp.L_shift((0, 0, 0), order, symbols, L_dict, source_order=source)
        == L[0] + x * L[1] + y * L[2] + z * L[3]
    )
...
def test_L_shift_1_order_dipole_source():
    ...
    L_dict, _ = gen.generate_mappings(
        order - source, symbols, key="grevlex", source_order=0
    )
